defmodule Slackkit.Bot.Event.Monitor do
  ...
  def handle_info({:gen_event_EXIT, _handler, reason}, {manager, handler, initial}) when reason in [:normal, :shutdown] do
    { :stop, reason, {manager, handler, initial} }
  end
  
  def handle_info({:gen_event_EXIT, _handler, _reason}, {manager, handler, initial}) do
